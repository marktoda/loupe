...
pub fn render_run_list(frame: &mut Frame, area: Rect, app: &mut App) {
    let items: Vec<ListItem> = app.runs.iter().map(|run| {
        ...
        let duration = run.duration()
            .map(|d| {
                ...
                if mins > 0 { format!("{mins}m") } else { format!("{}s", d.num_seconds()) }
            })
            .unwrap_or_default();
        ...
        let detail = Line::from(vec![
            Span::raw("   "),
            Span::styled(format!("{duration} {status_label}"), Style::default().fg(Color::DarkGray)),
        ]);
    ...
    let list = List::new(items)
        .block(Block::default().title(" Runs ").borders(Borders::ALL).border_style(Style::default().fg(Color::DarkGray)))
